...
class DeepseekV4AttentionMetadataBuilder(CommonAttentionBuilder):
    ...
    def build_kv_cache_tensor(self, layer_id: int, module):
        """Bind V4 module views; delegate unknown modules to the common path."""
        # Lazy import keeps the data-plane helper out of the backend/model import
        # cycle.  The helper itself keeps DeepSeek model-type imports lazy too.
        from atom.model_ops.attentions.dsv4.kv_bind import bind_kv_cache_tensor

        if bind_kv_cache_tensor(self, layer_id, module):
            return None
        # Explicit call is intentional: this fallback must not change if another
        # mixin is inserted into the builder's MRO.
        return CommonAttentionBuilder.build_kv_cache_tensor(self, layer_id, module)
