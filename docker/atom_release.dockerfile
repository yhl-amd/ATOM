...
# storage extensions, but its bundled c_ops is built against the release
# PyTorch ABI.  ATOM images may carry a newer ROCm PyTorch, so compile only
# c_ops against the image's torch and replace the wheel copy.  Building the
# whole source package also compiles three unrelated common extensions and can
# exceed Docker builder memory.
# The v0.5.3 wheel also contains LMCache#3828's scheduler-heartbeat regression;
# the guarded source-file replacement below backports its two-line fix.
ARG LMCACHE_COMMIT=24159d6b3bb4c94fcc3e74b76cc8630f90764cc1
ARG LMCACHE_WHEEL_URL=https://github.com/LMCache/LMCache/releases/download/v0.5.3-rocm/lmcache-0.5.3-cp312-cp312-manylinux_2_35_x86_64.whl
ARG LMCACHE_WHEEL_SHA256=6406280fa2762a6314018926d4f2d4178e05f605a32bfdb1971b85bacd1ebae4
...
RUN echo "========== [ATOM] LMCache HIP c_ops (${LMCACHE_COMMIT}, arch=${PYTORCH_ROCM_ARCH}) ==========" && \
    curl -fL "${LMCACHE_WHEEL_URL}" -o /tmp/lmcache-0.5.3-cp312-cp312-manylinux_2_35_x86_64.whl && \
    echo "${LMCACHE_WHEEL_SHA256}  /tmp/lmcache-0.5.3-cp312-cp312-manylinux_2_35_x86_64.whl" | sha256sum -c - && \
    "${VENV_PYTHON}" -m pip install --no-deps /tmp/lmcache-0.5.3-cp312-cp312-manylinux_2_35_x86_64.whl && \
    ...
    LMCACHE_ADAPTER=/opt/LMCache/lmcache/integration/vllm/vllm_multi_process_adapter.py && \
    cmp "${LMCACHE_ADAPTER}" "${LMCACHE_SITE}/lmcache/integration/vllm/vllm_multi_process_adapter.py" && \
    test "$(grep -Fc 'if self._heartbeats is not None:' "${LMCACHE_ADAPTER}")" -eq 2 && \
    sed -i 's/if self\._heartbeats is not None:/if self._heartbeats:/' "${LMCACHE_ADAPTER}" && \
    test "$(grep -Fc 'if self._heartbeats:' "${LMCACHE_ADAPTER}")" -eq 2 && \
    cp "${LMCACHE_ADAPTER}" "${LMCACHE_SITE}/lmcache/integration/vllm/" && \
    cp /opt/LMCache/lmcache/c_ops*.so "${LMCACHE_SITE}/lmcache/" && \
    rm -f /tmp/lmcache-0.5.3-cp312-cp312-manylinux_2_35_x86_64.whl && \
    ...
    "${VENV_PYTHON}" -c "import glob, inspect, os, sysconfig, torch; torch.cuda.is_available = lambda: True; import lmcache, lmcache.c_ops; \
from lmcache.v1.cache_engine import LMCacheEngineBuilder; \
from lmcache.v1.memory_management import MemoryFormat; \
from lmcache.v1.lookup_client.factory import LookupClientFactory; \
from lmcache.v1.config import LMCacheEngineConfig; \
from lmcache.v1.metadata import LMCacheMetadata; \
from lmcache.integration.vllm.vllm_multi_process_adapter import LMCacheMPSchedulerAdapter, LMCacheMPWorkerAdapter; \
from lmcache.v1.multiprocess.group_view import EngineGroupInfo; \
c_ops_paths = glob.glob(os.path.join(sysconfig.get_path('purelib'), 'lmcache', 'c_ops*.so')); \
assert 'rocm' in torch.__version__, torch.__version__; \
assert c_ops_paths, 'LMCache HIP c_ops extension was not installed'; \
assert hasattr(lmcache.c_ops, 'execute_object_group_transfer'), 'c_ops fell back to torch backend'; \
assert 'self._heartbeats is not None' not in inspect.getsource(LMCacheMPSchedulerAdapter._ensure_heartbeat_started), 'LMCache scheduler heartbeat regression is present'; \
